...
//        DAMAGE_EFFECTS.add(PotionEffectType.HERO_OF_THE_VILLAGE);
    ...
    public static boolean isCrop(Material type) {
        ...
        if (type == Material.PUMPKIN || type == Material.MELON || type == Material.CACTUS ||
                type == Material.SUGAR_CANE || type == Material.BAMBOO || type == Material.BAMBOO_SAPLING ||
    ...
    public static boolean isUseFlagApplicable(Material material) {
        if (Tag.BUTTONS.isTagged(material) || Tag.DOORS.isTagged(material) ||
                Tag.TRAPDOORS.isTagged(material) || Tag.FENCE_GATES.isTagged(material) ||
                Tag.PRESSURE_PLATES.isTagged(material)) {
            return true;
        }
        // yea, that's not all, there are some more
        if (material == Material.LEVER || material == Material.LECTERN || material == Material.ENCHANTING_TABLE ||
                material == Material.BELL || material == Material.LOOM || material == Material.CARTOGRAPHY_TABLE ||
                material == Material.STONECUTTER || material == Material.GRINDSTONE || material == Material.VAULT) {
            return true;
        }
        return false;
    }
    ...
    public static boolean isArmor(Material type) {
        ...
        // yea, that's not all, there are some more
        if (type == Material.CARVED_PUMPKIN || type == Material.ELYTRA) {
            return true;
        }
        return false;
    }
    ...
    public static boolean isToolApplicable(Material toolMaterial, Material targetMaterial) {
        if (toolMaterial == Material.WOODEN_HOE || toolMaterial == Material.STONE_HOE ||
                toolMaterial == Material.IRON_HOE || toolMaterial == Material.GOLDEN_HOE ||
                toolMaterial == Material.DIAMOND_HOE || toolMaterial == Material.NETHERITE_HOE) {
            if (targetMaterial == Material.GRASS_BLOCK || targetMaterial == Material.DIRT ||
                    targetMaterial == Material.DIRT_PATH || targetMaterial == Material.ROOTED_DIRT) {
                return true;
            }
            return false;
        }

        if (toolMaterial == Material.WOODEN_AXE || toolMaterial == Material.STONE_AXE ||
                toolMaterial == Material.IRON_AXE || toolMaterial == Material.GOLDEN_AXE ||
                toolMaterial == Material.DIAMOND_AXE || toolMaterial == Material.NETHERITE_AXE) {
            if (isWaxedCopper(targetMaterial)) return true;
            if (Tag.LOGS.isTagged(targetMaterial)) return true;
            if (targetMaterial == Material.OAK_WOOD || targetMaterial == Material.DARK_OAK_WOOD ||
                    targetMaterial == Material.ACACIA_WOOD || targetMaterial == Material.BIRCH_WOOD ||
                    targetMaterial == Material.SPRUCE_WOOD || targetMaterial == Material.PUMPKIN ||
                    targetMaterial == Material.BAMBOO_BLOCK || targetMaterial == Material.JUNGLE_WOOD ||
                    targetMaterial == Material.CRIMSON_STEM || targetMaterial == Material.WARPED_STEM ||
                    targetMaterial == Material.CRIMSON_HYPHAE || targetMaterial == Material.WARPED_HYPHAE) {
                return true;
            }
            return false;
        }

        if (toolMaterial == Material.WOODEN_SHOVEL || toolMaterial == Material.STONE_SHOVEL ||
                toolMaterial == Material.IRON_SHOVEL || toolMaterial == Material.GOLDEN_SHOVEL ||
                toolMaterial == Material.DIAMOND_SHOVEL || toolMaterial == Material.NETHERITE_SHOVEL) {
            if (targetMaterial == Material.GRASS_BLOCK || targetMaterial == Material.CAMPFIRE ||
        ...
        if (toolMaterial == Material.SHEARS) {
            if (targetMaterial == Material.PUMPKIN || targetMaterial == Material.BEE_NEST ||
        ...
        if (toolMaterial == Material.BLACK_DYE || toolMaterial == Material.BLUE_DYE ||
                toolMaterial == Material.BROWN_DYE || toolMaterial == Material.CYAN_DYE ||
                toolMaterial == Material.GRAY_DYE || toolMaterial == Material.GREEN_DYE ||
                toolMaterial == Material.LIGHT_BLUE_DYE || toolMaterial == Material.LIGHT_GRAY_DYE ||
                toolMaterial == Material.LIME_DYE || toolMaterial == Material.MAGENTA_DYE ||
                toolMaterial == Material.ORANGE_DYE || toolMaterial == Material.PINK_DYE ||
                toolMaterial == Material.PURPLE_DYE || toolMaterial == Material.RED_DYE ||
                toolMaterial == Material.WHITE_DYE || toolMaterial == Material.YELLOW_DYE ||
                toolMaterial == Material.GLOW_INK_SAC || toolMaterial == Material.INK_SAC) {
            return Tag.ALL_SIGNS.isTagged(targetMaterial);
        }

        if (toolMaterial == Material.HONEYCOMB) {
            if (isUnwaxedCopper(targetMaterial) || Tag.ALL_SIGNS.isTagged(targetMaterial)) {
                return true;
            }
            return false;
        }

        if (toolMaterial == Material.BRUSH) {
            if (targetMaterial == Material.SUSPICIOUS_GRAVEL || targetMaterial == Material.SUSPICIOUS_SAND) {
                return true;
            }
            return false;
        }

        if (toolMaterial == Material.WRITTEN_BOOK || toolMaterial == Material.WRITABLE_BOOK) {
            if (targetMaterial == Material.LECTERN) {
                return true;
            }
            return false;
        }
    ...
    public static boolean isUnwaxedCopper(Material type) {
        if (type == Material.COPPER_BLOCK || type == Material.CHISELED_COPPER || type == Material.COPPER_DOOR ||
                type == Material.COPPER_TRAPDOOR || type == Material.COPPER_GRATE || type == Material.COPPER_BULB) {
            return true;
        }
        // yea, that's not all, there are some more
        String typeName = type.name();
        if (typeName.startsWith("EXPOSED_") || typeName.startsWith("WEATHERED_") ||
                typeName.startsWith("OXIDIZED_") || typeName.startsWith("CUT_COPPER")) {
            return true;
        }
        return false;
    }
